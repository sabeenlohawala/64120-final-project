...
import similarity_model as sim
...
def get_series_cards(i, series_cards_values, series_cards_suits):
    ...
    """
    return [(series_cards_values['series' + str(i)][j], series_cards_suits['series' + str(i)][j]) for j in
            range(series_cards_values['series' + str(i)].count())]
...
def build_series_info(all_responses_filename, cards_values_filename, cards_suits_filename):
    ...
    for i in range(1, 10):
        ...
        series_dict['similarity_wa1_wb1'] = sim.representativeness_similarity(series_dict['cards'])
...
def optimize_sim_corrcoef(series_info, num_steps=10, mean_or_mode='mean'):  # optimize average corrcoef
    """
    Finds the prior_h1 and prior_h2 settings that result in the maximum avg corrcoef for the three hypotheses.
    ...
    for wa in [i / num_steps for i in range(0, num_steps + 1)]:
        for wb in [i / num_steps for i in range(0, num_steps + 1)]:
            if wa + wb <= 2:
                print(wa,wb)
                all_sim = [sim.representativeness_similarity(series_info[key]['cards'], wa, wb) for key in series_info.keys()]
                sim_h1 = [arr[0] for arr in all_sim]
                sim_h2 = [arr[1] for arr in all_sim]
                sim_h3 = [arr[2] for arr in all_sim]

                corr_h1 = np.corrcoef(mean_h1, sim_h1)[0][1]
                corr_h2 = np.corrcoef(mean_h2, sim_h2)[0][1]
                corr_h3 = np.corrcoef(mean_h3, sim_h3)[0][1]
                avg_corr = (corr_h1 + corr_h2 + corr_h3) / 3
                if not max_wa or not max_wb or avg_corr > max_corrcoef:
                    max_corrcoef = avg_corr
                    max_wa = wa
                    max_wb = wb
    return max_corrcoef, max_wa, max_wb
...
def calculate_likelihood_corrcoef(series_info, mean_or_mode='mean'):
    ...
    likelihood_h1, likelihood_h2, likelihood_h3 = [[series['likelihood'][0] for key, series in series_info.items()] for
                                                   i in range(3)]
    return (np.corrcoef(mean_h1, likelihood_h1)[0][1] + np.corrcoef(mean_h2, likelihood_h2)[0][1] +
            np.corrcoef(mean_h3, likelihood_h3)[0][1]) / 3


def main():
    series_info = build_series_info('./data/google_form_responses.csv', './data/series_cards_values.csv',
                                    './data/series_cards_suits.csv')
    ...
    max_sim_corrcoef, max_wa, max_wb = optimize_sim_corrcoef(series_info, 10, 'mean')
    print(max_sim_corrcoef)
    print(max_wa)
    print(max_wb)
